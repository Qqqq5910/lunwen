...
from analyzer.docx_edit import convert_generated_ref_fields_to_hyperlinks
...
def analyze_document_state(document, school_rules=None):
    ...
    issues = []
    issues.extend(check_citation_format(citations))
...
def analyze_docx(file_path, school_requirement_path=None, fix_superscript=False, fix_citation_ranges=False, fix_school=False, keep_history=True, job_id=None, report_base_dir="reports"):
    ...
    fixed_info = {"enabled": False, "plain_citation_fixed_count": 0, "superscript_fixed_count": 0, "citation_range_fixed_count": 0, "school_format_fixed_count": 0, "whitespace_fixed_count": 0, "hyperlink_converted_count": 0, "output_docx": None, "latest_docx": None, "download_url": None}
    ...
    if fix_superscript or fix_citation_ranges or fix_school:
        ...
        try:
            ...
            whitespace_fixed_count = 0
        try:
            hyperlink_converted_count = convert_generated_ref_fields_to_hyperlinks(final_document)
        except Exception:
            hyperlink_converted_count = 0
        before_state = analyze_document_state(final_document, school_rules)
